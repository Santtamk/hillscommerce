    ...
    <>
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        ...
        <div className="mb-12 flex flex-col items-center space-y-8">
          ...
          {/* Category Buttons */}
          <FadeIn delay={0.2} direction="up" className="flex flex-wrap justify-center gap-3 md:gap-4">
            {CATEGORIES.map((category) => (
              
              <button
                key={category}
                onClick={() => handleCategoryChange(category)}
                className={`rounded-full mx-1 border px-5 py-2 text-xs md:text-sm font-medium uppercase tracking-wider transition-all
                  ${
        ...
        {/* Pagination */}
        {totalPages > 1 && (
          <FadeIn delay={0.4} direction="up" className="mt-20 flex justify-center gap-2">
            <button
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                ...
                className={`min-w-[40px] rounded-md border px-4 py-2 text-sm font-medium transition-colors mx-1 ${
                  currentPage === page
                    ? "border-pine bg-pine text-white"
        ...
        {filteredProducts.length === 0 && (
          <FadeIn className="flex h-64 w-full flex-col items-center justify-center space-y-4 text-pine/50">
